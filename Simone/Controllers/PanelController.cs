...
namespace Simone.Controllers
{
    ...
    public class PanelController : Controller
    {
        ...
        [Route("Panel/Reportes")] // fija la ruta y evita colisiones
        public async Task<IActionResult> Reportes(CancellationToken ct)
        {
            var ventas = await _context.Ventas
                .AsNoTracking()
                .Include(v => v.Clientes) // en tu modelo la navegación es "Clientes"
                ...
                .Select(v => new CompradorResumenVM
                {
                    ...
                    Nombre = v.Clientes != null ? v.Clientes.Nombre : "(sin cliente)",
                    Email = v.Clientes != null ? v.Clientes.Email : null,
                    Telefono = v.Clientes != null ? v.Clientes.Telefono : null,
                    Fecha = v.FechaVenta,
                    Estado = v.Estado,
            ...
            return View(ventas); // Views/Panel/Reportes.cshtml
        }

        // Detalle con info de persona + envío + productos
        [HttpGet]
        [Authorize(Roles = "Administrador")]
        [Route("Panel/VentaDetalle/{id:int}")]
        public async Task<IActionResult> VentaDetalle(int id, CancellationToken ct)
        {
            ...
            var dirs = await _context.DireccionesCliente
                .AsNoTracking()
                .Where(d => d.ClienteID == v.ClienteID)
                .OrderByDescending(d => d.FechaRegistro)
                .ToListAsync(ct);

            var model = new VentaDetalleVM
            {
                VentaID = v.VentaID,
                Fecha = v.FechaVenta,
                Estado = v.Estado,
                ...
                ClienteID = v.ClienteID,
                Nombre = v.Clientes?.Nombre ?? "(sin cliente)",
                Email = v.Clientes?.Email,
                Telefono = v.Clientes?.Telefono,
                Direccion = v.Clientes?.Direccion,

                Direcciones = dirs.Select(d => new DireccionVM
                {
                    DireccionID = d.DireccionID,
                    Calle = d.Calle,
                    Ciudad = d.Ciudad,
                    EstadoProvincia = d.EstadoProvincia,
                    CodigoPostal = d.CodigoPostal,
                    TelefonoContacto = d.TelefonoContacto,
                    FechaRegistro = d.FechaRegistro
                }).ToList(),

                Detalles = v.DetalleVentas.Select(d => new DetalleFilaVM
                {
                    Producto = d.Producto != null ? d.Producto.Nombre : $"#{d.ProductoID}",
                    Cantidad = d.Cantidad,
                    Subtotal = d.Subtotal ?? (d.Cantidad * d.PrecioUnitario)
                }).ToList()
            };

            return View("~/Views/Panel/VentaDetalle.cshtml", model);
        }
        ...
        public async Task<IActionResult> MarcarEnviada(int id, CancellationToken ct)
        {
            ...
            return !string.IsNullOrWhiteSpace(referer) ? Redirect(referer) : RedirectToAction(nameof(Reportes));
        }
